...
class LorryReceiptList(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        """
        # First check if the user type is either developer, admin or accounts, 
        # share all the LR files. Else share only the LR generated by him.
        if (request.user.user_type==User.developer or request.user.user_type==User.admin \
             or request.user.user_type==User.accounts):
